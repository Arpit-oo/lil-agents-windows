...
export class GeminiSession extends BaseSession {
  ...
  send(message: string): void {
    ...
    (async () => {
      const binaryPath = await findBinary('gemini', this.customBinaryPath);
      if (!binaryPath) { this.callbacks?.onError('Gemini CLI not found.'); return; }
      const args = ['--yolo'];
      if (this.history.length > 1) args.push('--resume', 'latest');
      args.push(message);
      ...
      proc.on('error', (err: Error) => { this.callbacks?.onError(`Failed to run Gemini: ${err.message}`); });
    })();
  }
